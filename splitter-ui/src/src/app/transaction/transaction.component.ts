...
export class TransactionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.roomMateController.getRoomMatesUsingGET(this.credentialService.credentials.username).subscribe((users) => {
      this.users = users;
      if (this.eventId) {
        this.eventControlerService.findEventByIdUsingGET(this.eventId).subscribe((event) => {
  ...
  private saveExpense(event: EventVO) {
    ...
    this.eventControlerService.createEventUsingPOST(eventToSave).subscribe((e) => {
  ...
  private createEventForm(event?: EventVO) {
    ...
    return this.formBuilder.group({
      amountSpent: new FormControl(event ? event.amountSpent : null, [
        Validators.required,
        Validators.pattern(/^\d{1,10}(\.\d{1,4})?$/),
      ]),
