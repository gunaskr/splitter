...
export class MyRoommatesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.roomMateController.getRoomMatesUsingGET(this.credentialService.credentials.username).subscribe((users) => {
      this.users = users;
      const userFormGroups = users
      .filter(user => user.mobileNo !== this.credentialService.credentials.username) /* do not add self numer */
      .map((user) => this.createUserGroup(user));
  ...
  saveDetails() {
    ...
    if (formGroupsToSave.length) {
      ...
      this.roomMateController.createRoomMatesUsingPOST(roomMates).subscribe((response) => {
  ...
  deleteUser(index: number) {
    ...
    if (mobileNo && id) {
      this.roomMateController
        .deleteRoomMateUsingDELETE(this.credentialService.credentials.username, mobileNo)
        .subscribe((response) => {
